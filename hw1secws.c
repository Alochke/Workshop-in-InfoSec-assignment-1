/*
    ...
    Conceptually, the idea is comes from the facts that:
    ...
    Those we create three diffrent hooks, first two are hooked into NF_IP_FORWARD and NF_IP_LOCAL_OUT, handling allowed packets, and third is hooked into NF_IP_FORWARD handling the blocked ones.
*/
...
#define HOOKS_NUM 3 /* The number of nf_hook_ops struts We'll use. */
...
static unsigned int nf_forward_fn(void* priv, struct sk_buff *skb, const srruct nf_hook_ops *state)
{
...
static unsigned int nf_local_fn(void* priv, struct sk_buff *skb, const srruct nf_hook_ops *state)
{
    ...
    for(size_t i = 0; i < HOOKS_NUM; i++)
    {
        ...
        default:
            ...
            case IN:
                ...
                break;
            default:
                hook[i]->hooknum = NF_IP_LOCAL_OUT;
                break;
            }
        }
        ERR_CHECK(int err = (nf_register_net_hook(&init_net, hook[i]) < 0), destroy_hooks(i - 1), "nf_register_net_hook", err);
    }
}
